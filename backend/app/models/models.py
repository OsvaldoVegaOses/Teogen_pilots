from sqlalchemy import Column, String, Text, Boolean, Integer, ForeignKey, JSON, Float, DateTime, Table
...
class Interview(Base):
    ...
    transcription_method = Column(String(50))  # ← ADDED: 'axial-speech', 'gpt-4o-transcribe-diarize', etc.
    full_text = Column(Text)
    word_count = Column(Integer)
    language = Column(String(10))
    speakers = Column(JSON, default=[])  # ← ADDED: list of speaker segments from diarization
    created_at = Column(DateTime, default=datetime.utcnow)
...
class Memo(Base):
    ...
    interview_id = Column(UUID(as_uuid=True), ForeignKey("interviews.id", ondelete="SET NULL"), nullable=True)  # ← ADDED
    code_id = Column(UUID(as_uuid=True), ForeignKey("codes.id", ondelete="SET NULL"), nullable=True)  # ← ADDED
    ...
    interview = relationship("Interview", foreign_keys=[interview_id])  # ← ADDED
    code = relationship("Code", foreign_keys=[code_id])  # ← ADDED
...
class Theory(Base):
    ...
    generated_by = Column(String(100))  # ← ADDED: e.g. 'DeepSeek-V3.2-Speciale/Kimi-K2.5'
